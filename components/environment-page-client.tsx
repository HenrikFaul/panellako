...
function PollutantBar({ label, value, max, color }: { label: string; value: number | null; max: number; color: string }) {
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <span className="w-9 shrink-0 text-right text-[9px] tabular-nums text-slate-400">{value.toFixed(1)}</span>
    </div>
  );
}
...
function SolarCalculator({ solar }: { solar: SolarData }) {
  ...
  return (
    <div className="space-y-5">
      <div>
        <p className="mb-2 text-[9px] font-bold uppercase tracking-widest text-slate-500">Havi termelés (kWh/kWp)</p>
        <div className="flex items-end gap-1 h-16">
          {solar.monthly.map(m => (
            <div key={m.month} className="flex flex-1 flex-col items-center gap-0.5">
              ...
              <span className="text-[7px] text-slate-600">{m.month}</span>
            </div>
          ))}
        </div>
      </div>
    ...
    <div className="min-h-screen">
      ...
      <main className="mx-auto max-w-6xl space-y-4 px-4 py-6 sm:px-6">
        ...
        <Section id="sec-air">
          <button type="button" onClick={() => setAqExpanded(e => !e)}
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="rounded-full border border-white/[0.07] px-2.5 py-0.5 text-[9px] text-slate-500">Open-Meteo · CAMS</span>
            ...
            <div style={{ overflow: 'hidden' }}>
                ...
                <div className="flex flex-col gap-5 border-b border-white/[0.06] p-6 lg:border-b-0 lg:border-r">
                  {loadingAq ? (
                    <div className="flex flex-col items-center gap-4 animate-pulse">
                      ...
                      {[1,2,3,4].map(i => <div key={i} className="h-3 w-full rounded bg-white/[0.06]" />)}
                    </div>
                  ) : aq ? (
                    <>
                      ...
                      {aq.source === 'mock' && <p className="text-center text-[9px] text-slate-700">Offline mód — közelítő adatok</p>}
                    </>
                  ) : (
              ...
              {aq && aq.hourly24.length > 0 && (
                <div className="border-t border-white/[0.06] px-6 py-5">
                  <p className="mb-3 text-[9px] font-bold uppercase tracking-widest text-slate-500">24 órás trend</p>
              ...
              {aq && aq.daily.length > 0 && (
                <div className="border-t border-white/[0.06] px-6 py-5">
                  <p className="mb-3 text-[9px] font-bold uppercase tracking-widest text-slate-500">PM2.5 előrejelzés (7 nap)</p>
                  <div className="flex flex-col gap-2">
                    {aq.daily.slice(0, 7).map(d => (
                      <div key={d.date} className="flex items-center gap-3">
                        <span className="w-6 shrink-0 text-[9px] font-bold text-slate-500">{dayLabel(d.date)}</span>
                ...
                <div className="grid gap-3 sm:grid-cols-3">
                  ...
                  <div className="rounded-2xl border border-white/[0.07] bg-white/[0.02] p-4">
                    ...
                    <ul className="space-y-1 text-[9px] leading-relaxed text-slate-500">
                      ...
                      <li><span className="text-slate-400 font-semibold">Hőinverzió:</span> éjszakai felhalmozódás</li>
                    </ul>
                  </div>
                  <div className="rounded-2xl border border-white/[0.07] bg-white/[0.02] p-4">
        ...
        <Section id="sec-pollen">
          <SectionHeader icon={<Flower2 size={18} className="text-purple-400" />} title="Pollen & UV" source="Open-Meteo · CAMS · WHO" />
          <div className="p-6 space-y-6">
            {loadingAq || loadingWeather ? (
              <div className="space-y-3 animate-pulse">{[1,2,3].map(i=><div key={i} className="h-8 rounded-xl bg-white/[0.06]" />)}</div>
            ) : (
              <div className="grid gap-6 lg:grid-cols-2">
                ...
                <div>
                  ...
                  {aq ? (
                    ...
                    />
                  ) : <p className="text-[11px] text-slate-600">Pollenadatok nem elérhetők</p>}
                </div>
                {/* UV + Wind */}
                <div>
                  ...
                  {weather ? (
                    ...
                    />
                  ) : <p className="text-[11px] text-slate-600">Időjárásadatok nem elérhetők</p>}
                </div>
              </div>
            )}
            ...
            {weather && weather.daily.length > 0 && (
              <div className="border-t border-white/[0.06] pt-5">
                <p className="mb-3 text-[9px] font-bold uppercase tracking-widest text-slate-500">7 napos időjárás-előrejelzés</p>
              ...
              {loadingGreen ? (
                ...
                <>
                  ...
                  <div className="rounded-2xl border border-white/[0.07] bg-white/[0.02] p-4">
                    ...
                    <div className="flex items-center gap-3">
                      <span className="text-[9px] text-slate-600 shrink-0">Zajos</span>
                ...
                <div className="flex flex-col items-center gap-2 py-8">
                  ...
                  <p className="text-[11px] text-slate-600">Zöldfelület-adatok betöltése…</p>
                  <p className="text-[9px] text-slate-700">Az első lekérdezés ~30 másodpercet vehet igénybe</p>
                </div>
              )}
            </div>
          </Section>
        ...
        <div ref={solarRef}>
          <Section id="sec-solar">
            <SectionHeader icon={<Zap size={18} className="text-amber-400" />} title="Napenergia-potenciál" source="EU JRC PVGIS" />
            <div className="p-6">
              {loadingSolar ? (
                <div className="space-y-3 animate-pulse"><div className="h-28 rounded-2xl bg-white/[0.06]" /><div className="h-16 rounded-2xl bg-white/[0.06]" /></div>
              ) : errorSolar ? (
                <p className="text-center text-[11px] text-slate-600 py-6">PVGIS-adat jelenleg nem elérhető</p>
              ) : solar ? (
                ...
                <div className="flex flex-col items-center gap-2 py-8">
                  ...
                  <p className="text-[11px] text-slate-600">Napenergia-adatok betöltése…</p>
                </div>
              )}
            </div>
          </Section>
