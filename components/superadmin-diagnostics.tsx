...
function badgeColor(status: number | null, ok: boolean): string {
  if (status === null) return 'bg-rose-500/10 text-rose-300 border-rose-500/30';
  if (ok) return 'bg-emerald-500/10 text-emerald-300 border-emerald-500/30';
  if (status >= 400 && status < 500) return 'bg-amber-500/10 text-amber-300 border-amber-500/30';
  ...
  return (
    <section className="rounded-2xl border border-white/[0.08] bg-white/[0.02] p-6">
      <div className="mb-4 flex items-start justify-between gap-4">
        <div>
          <h2 className="text-base font-bold text-slate-200">🛠 Diagnosztika — külső API curl</h2>
        ...
        <button
          ...
          className="shrink-0 rounded-xl border border-emerald-500/30 bg-emerald-500/10 px-4 py-2 text-[11px] font-bold text-emerald-300 hover:bg-emerald-500/20 disabled:opacity-50"
        >
      ...
      {batchResults.length > 0 && (
        <div className="mb-6 rounded-xl border border-emerald-500/20 bg-emerald-500/5 p-4">
          <p className="mb-3 text-[10px] font-bold uppercase tracking-widest text-emerald-300">Overpass mirror health</p>
          <table className="w-full text-[11px]">
            <thead>
              <tr className="text-left text-slate-500">
                <th className="pb-2">Mirror</th>
            ...
            <tbody>
              {batchResults.map(({ preset, resp: r }) => (
                <tr key={preset.id} className="border-t border-white/[0.04]">
                  <td className="py-2 font-mono text-slate-300">{new URL(preset.spec.url).hostname}</td>
                  <td className="py-2">
                    <span className={`inline-block rounded-md border px-2 py-0.5 text-[10px] font-bold ${badgeColor(r.status, r.ok)}`}>
                      {r.status ?? 'ERR'} {r.statusText ?? ''}
                    </span>
                  </td>
                  <td className="py-2 text-right tabular-nums text-slate-400">{r.elapsedMs} ms</td>
                  <td className="py-2 text-right tabular-nums text-slate-400">{fmtBytes(r.bodyBytes)}</td>
                  <td className="py-2 text-slate-500">{r.error ?? (r.ok ? 'OK' : r.bodyText.slice(0, 80))}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <div className="flex flex-wrap gap-2">
          {PRESETS.map(p => (
            <button
              ...
              title={p.description}
              className="rounded-xl border border-white/[0.1] bg-white/[0.03] px-3 py-1.5 text-[10px] text-slate-300 hover:bg-white/[0.08]"
            >
              {p.label}
            </button>
          ))}
      ...
      <div className="grid gap-3 md:grid-cols-12">
        <div className="md:col-span-2">
          <label className="block text-[9px] font-bold uppercase tracking-widest text-slate-500">Method</label>
          <select
            value={method}
            onChange={e => setMethod(e.target.value)}
            className="mt-1 w-full rounded-lg border border-white/[0.1] bg-white/[0.03] px-2 py-2 text-[11px] text-slate-200"
          >
            {['GET', 'POST', 'PUT', 'PATCH', 'DELETE', 'HEAD'].map(m => <option key={m} value={m}>{m}</option>)}
          </select>
        </div>
        <div className="md:col-span-8">
          <label className="block text-[9px] font-bold uppercase tracking-widest text-slate-500">URL</label>
          <input
            type="text"
            value={url}
            onChange={e => setUrl(e.target.value)}
            placeholder="https://overpass.kumi.systems/api/interpreter"
            className="mt-1 w-full rounded-lg border border-white/[0.1] bg-white/[0.03] px-3 py-2 font-mono text-[11px] text-slate-200"
          />
        </div>
        <div className="md:col-span-2">
          <label className="block text-[9px] font-bold uppercase tracking-widest text-slate-500">Timeout (ms)</label>
          <input
            type="number"
            min={500}
            max={25000}
        ...
        <div className="md:col-span-6">
          <label className="block text-[9px] font-bold uppercase tracking-widest text-slate-500">Headers (JSON)</label>
          <textarea
            value={headersStr}
            onChange={e => setHeadersStr(e.target.value)}
            rows={6}
            className="mt-1 w-full rounded-lg border border-white/[0.1] bg-white/[0.03] px-3 py-2 font-mono text-[10px] text-slate-200"
            placeholder='{ "Content-Type": "application/json" }'
          />
          {headersErr && <p className="mt-1 text-[10px] text-rose-400">{headersErr}</p>}
        </div>
        <div className="md:col-span-6">
          <label className="block text-[9px] font-bold uppercase tracking-widest text-slate-500">Body (csak POST/PUT/PATCH/DELETE)</label>
          <textarea
            value={body}
            onChange={e => setBody(e.target.value)}
            rows={6}
            disabled={method === 'GET' || method === 'HEAD'}
            className="mt-1 w-full rounded-lg border border-white/[0.1] bg-white/[0.03] px-3 py-2 font-mono text-[10px] text-slate-200 disabled:opacity-40"
            placeholder='data=...'
          />
        </div>
      </div>
      ...
      <div className="mt-4 flex items-center gap-3">
        <button
          ...
          onClick={run}
          disabled={running}
          className="rounded-xl border border-sky-500/30 bg-sky-500/10 px-5 py-2 text-[11px] font-bold text-sky-300 hover:bg-sky-500/20 disabled:opacity-50"
        >
          {running ? 'Futtatás...' : 'Futtatás'}
        </button>
        {resp && (
          <span className="text-[10px] text-slate-500">
            {resp.redirected && <>redirect → <span className="text-slate-300 font-mono">{resp.finalUrl}</span> · </>}
            content-type: <span className="text-slate-300 font-mono">{resp.contentType ?? '?'}</span>
          </span>
        )}
      </div>
      ...
      {resp && (
        <div className="mt-5 rounded-xl border border-white/[0.08] bg-white/[0.02] p-4">
          <div className="mb-3 flex items-center gap-3">
            <span className={`inline-block rounded-md border px-2 py-1 text-[10px] font-bold ${badgeColor(resp.status, resp.ok)}`}>
              {resp.status ?? 'ERR'} {resp.statusText ?? ''}
            </span>
            <span className="text-[10px] text-slate-500 tabular-nums">{resp.elapsedMs} ms</span>
            <span className="text-[10px] text-slate-500 tabular-nums">{fmtBytes(resp.bodyBytes)}</span>
            {resp.bodyTruncated && <span className="text-[10px] text-amber-400">(truncated)</span>}
          </div>

          {resp.error && (
            <div className="mb-3 rounded-lg border border-rose-500/30 bg-rose-500/5 p-3">
              <p className="text-[10px] font-bold uppercase tracking-widest text-rose-300">Error</p>
              <p className="mt-1 font-mono text-[11px] text-rose-200">{resp.error}</p>
            </div>
          )}

          <details className="mb-3" open={!resp.error}>
            <summary className="cursor-pointer text-[10px] font-bold uppercase tracking-widest text-slate-500">Response headers ({Object.keys(resp.responseHeaders).length})</summary>
            <pre className="mt-2 max-h-40 overflow-auto rounded-lg bg-black/40 p-3 font-mono text-[10px] text-slate-300">
              {Object.entries(resp.responseHeaders).map(([k, v]) => `${k}: ${v}`).join('\n')}
            </pre>
          </details>

          <details open={!resp.error}>
            <summary className="cursor-pointer text-[10px] font-bold uppercase tracking-widest text-slate-500">Body ({fmtBytes(resp.bodyBytes)}{resp.bodyTruncated ? ', truncated' : ''})</summary>
            <pre className="mt-2 max-h-96 overflow-auto rounded-lg bg-black/40 p-3 font-mono text-[10px] text-slate-300 whitespace-pre-wrap break-all">
              {resp.bodyText || '(empty)'}
            </pre>
          </details>
        </div>
